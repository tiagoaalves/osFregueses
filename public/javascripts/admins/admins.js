...
function listAdmins() {
    ...
    .then(function(data) {
      ...
      $.each(data.response, function(index, value) {    
          ...
          //newRow.insertCell(0).appendChild(document.createTextNode(value.imagem));
  ...
  function updateUser1(data){
    
  swal({
  title: "Alterar Administrador",
  html:
    '<form id="formNewUser" name="formNewUser" action="" onfocus="deleteUser()">'+
    `<input id="swal-input1" class="swal2-input" placeholder="Número de Sócio" type="number" onfocus="validator1()">`+
    '<div id="socio"></div>'+
    '<input id="swal-input2" class="swal2-input" placeholder="Nome" onfocus="validatorNome()">'+
    '<div id="nome"></div>'+
  ...
  function deleteUser2(user){
    ...
    swal({title: "Administrador eliminado com sucesso!", type:"success"});
    listAdmins();
    }
          ...
          if(idade == morada){
            if(morada == password){
              if(password == confirmPassword){
                if(confirmPassword == ""){
                  updateUser3(document.getElementById("swal-input1").value);
                }else{
  ...
  function saveUser() {
    ...
    swal({title: "Administrador alterado com sucesso!", type:"success"});
    listAdmins();
    }
    }).then(function (result) {console.log(result);
    }).catch(function (err) {alert("Submission error"); console.error(err);
